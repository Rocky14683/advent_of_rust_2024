...
impl Kid {
    pub fn parse_row(csv_row: &str) -> Result<Kid, &'static str> {
        ...
        let mut field = csv_row.split(',');
        let name = field.next().unwrap().to_string();
        let good_deeds = field.next().ok_or("Missing field")?.parse().map_err(|_| "Invalid good deeds")?;
        let bad_deeds = field.next().ok_or("Missing field")?.parse().map_err(|_| "Invalid bad deeds")?;
